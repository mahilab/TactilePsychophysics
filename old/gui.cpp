...
public:
    ...
    /// Constructor
    TfxDemo() : ms("squeeze"), chCB(NUM_CH, false), plot() { 
    ...
    void update() override {    
        ...
        ImGui::SetNextWindowSize(Vector2f(WIDTH-10,84), ImGuiCond_Always);
        ImGui::Begin("Squeeze Control", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize); 
        if (powered)
              ImGui::PushStyleColor(ImGuiCol_Text, Greens::Chartreuse);
        ...
        ImGui::SliderFloat("Squeeze Level", &squeeze, 0.0f, 100.0f);
        ImGui::End();

        if (powered)
            ms.write_data({(double)(squeeze/100.0f)});

        ImGui::SetNextWindowPos(Vector2f(5,94), ImGuiCond_Always);

        ImGui::SetNextWindowSize(Vector2f(WIDTH-10,HEIGHT-100), ImGuiCond_Always);
        ImGui::Begin("Vibrotactor Control", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);   
        //------------------------------------------------------------------
        ...
        //------------------------------------------------------------------
        ImGui::Text("Waveform");
        ImGui::DragInt("Carrier", &freq, 0.5f, 0, 250, "%i Hz");
        ...
        ImGui::PushStyleColor(ImGuiCol_PlotLines, Blues::LightSkyBlue);
        ImGui::PushStyleColor(ImGuiCol_PlotLinesHovered, Blues::LightSkyBlue);
        auto title = str(durMs, "ms /", samples, " samples");
        ImGui::PlotLines("", &plot[0], (int)plot.size(),  0, title.c_str(), -1.0f, 1.0f, ImVec2(0,105));
        ImGui::PopStyleColor();
        ImGui::PopStyleColor();
        ImGui::PopItemWidth();
        ImGui::End();
