...
public:
    ...
    enum Mode       { Idle, Vibration, Squeeze };
    ...
    void update() override {
        ...
        // studies
        if (ImGui::Button("Vibration Study",ImVec2(-1,0))) {
            buildIdTrials();
            // saveIdStudy();
            start_coroutine(idStudy());
        }
        if (ImGui::Button("Squeeze Study",ImVec2(-1,0))) {
            buildJndTrials();
            // saveJndStudy();
            start_coroutine(jndStudy());
        }
        ImGui::EndDisabled();
    ...
    static constexpr int n_id_durations     = 2;
    static constexpr int n_id_vts           = 6;
    static constexpr int n_id_repetitions   = 5;
    static constexpr int n_id_windows       = 4;
    static constexpr int n_id_trials_window = n_id_durations * n_id_vts * n_id_repetitions;
    ...
    double m_id_durations[n_id_durations]       = {0.05, 0.25};   // 50 ms, 250 ms
    ...
    void buildIdTrials() {
        ...
        for (int f = 0; f < n_id_force_levels; ++f) {
            ...
            for (int w = 0; w < n_id_windows; ++w) {
                ...
                for (int d = 0; d < n_id_durations; ++d) {
                    for (int v = 0; v < n_id_vts; ++v) {
                        for (int r = 0; r < n_id_repetitions; ++r) {
                            ...
                            trial.duration    = m_id_durations[d];
    ...
    Enumerator idStudy() {
        ...
        for (int i = 0; i < order.size(); ++i) {
            ...
            for (auto& trial : m_id_trials[f]) {
                int response = -1;
                tact::Signal stimulus = tact::Sine(170) * tact::Envelope(trial.duration);
                ...
                csv.write_row(trial.num, trial.gen_num, trial.force, trial.window, trial.duration, trial.vt_stimulus, trial.vt_response);
                co_yield yield_time(1_s);
            }
